...
class ImportGraphState(TypedDict):
    ...
    states 的作用
    在 LangGraph 框架中， state （状态）有以下核心作用：
    1. 节点间数据共享 ：所有节点都操作同一个 state 对象，节点 A 输出的数据，节点 B 可以直接读取。
    2. 工作流记忆 ：记录整个流程的中间结果，比如 PDF 路径、MD 内容、分块结果等。
    3. 类型安全 ：通过 TypedDict 提供自动补全和类型检查，避免拼写错误。
    4. 流程控制 ：通过状态中的开关字段（如 is_pdf_read_enabled ）决定走哪个分支。
    """
