...
**
1. 配置校验**
2. **获取上传链接 + batch_id**（批量接口自动生成，无需手动提交） → 
3. **文件上传**（含重试 + 代理禁用） → 
4. **轮询状态**（根据 batch_id 持续查询，直至完成 / 失败 / 超时）
"""
def step_2_upload_and_poll(pdf_path_obj: Path, output_dir_obj: Path):
    ...
    # 提取核心数据：上传链接和任务唯一标识
    signed_url = resp_data["data"]["file_urls"][0] # ← 预签名上传链接
    batch_id = resp_data["data"]["batch_id"]   # ← 任务追踪ID
