...
def node_search_embedding_hyde(state):
    ...
    **什么是HyDE**: HyDE（Hypothetical Document Embeddings，“假设文档向量化”）是一种让检索更“聪明”的技巧。
    当用户只给出一句短问题时，系统先不急着去向量库里硬搜，
    而是先让大模型“脑补”出一段可能的答案或相关描述——就像先写一段“假想的说明书”。
    这段文字通常更具体、更像一篇小文档，包含更多关键词、背景和表达方式。接着，把这段“假想文档”做成向量，
    再拿它去向量库里找相似内容。结果往往比直接拿那一句短问题去做向量检索更准，因为检索时用的是更“饱满”的语义信号。
    核心逻辑是 先让 LLM 虚构一个“理想答案”，再用这个答案去向量库检索真实的文档 。
    一句话总结： 
    它通过“LLM 生成假设性答案”来增强原始问题的语义信息，再进行混合向量检索，
    从而大幅提升对“语义匹配但字面不匹配”问题的召回能力。
    它的优点是简单、通用、对很多场景都有效；缺点是也会“脑补过头”，
    生成的假设文本可能带偏方向，所以常见做法是配合过滤、重排或多路检索一起用。
     执行步骤：
