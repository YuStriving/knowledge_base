...
def node_rerank(state):
    ...
    场景：由于 RRF 只能对于同源的检索结果进行打分筛选，但应对不同源的（比如：切片与网页搜索结果）就力不从心了。这时就需要 Rerank 出场了。
    机制：相对 RRF 这种只依靠一个公式就能打分，Rerank 就比较复杂耗时了，Rerank 的是一种依靠 AI 模型精读打分，
    它会仔细阅读文章与问题得出相关性评分，这与大模型的机制非常接近。
    """
    print("---Rerank---")
...
def step_3_topk(scored_docs):
    ...
    它遍历文档列表，计算相邻两个文档分数的 差距 (Gap) 。
    如果差距过大，说明“后面的文档质量骤降”，立刻停止截取。
